...
queue_process_t *processo_corrente, *processo_anterior ;
...
//        libera o processador
			processo_anterior = processo_corrente;
			processo_corrente = NULL;
			pthread_mutex_unlock(&processador);
//      fim se
		}
...
//        escolhe uma tarefa da fila de prontas
				...
				iterador = ready;
				for(i = 0; i < queue_size((queue_t *) ready); i++){
					if (aux->prioridade_estatica < iterador->prioridade_estatica){
						aux = iterador;
						iterador = iterador->next;
					}else iterador = iterador->next;
				}
//        migra essa tarefa para o estado "rodando"
				...
				if (processo_anterior != processo_corrente) numero_troca_contexto++;
//      fim se
			}
//    fim se
		}
