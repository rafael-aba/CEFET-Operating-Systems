...
void read_input(){
	...
	while (scanf("%d %d %d",&a,&b,&c) != -1) {
		...
		process->tempo_executado_quantum_atual = 0;
		...
		if (aux != NULL){
			switch (aux->estado_atual){ // 0 - nova, 1 - pronta, 2 - rodando, 3 - terminada
				...
				case 1:
					...
					break;
				case 2:
					aux->tempo_executado_quantum_atual++;
					printf("\t##");
					break;
				case 3:
					printf("\t");
...
//        se a tarefa rodando chegou ao fim de seu quantum
			if(processo_corrente->tempo_executado_quantum_atual == 2){
//        migra a tarefa para a fila de prontos
				processo_corrente->estado_atual = 1;
				queue_append((queue_t **) &ready, (queue_t *) processo_corrente);
//        libera o processador
				processo_corrente = NULL;
				pthread_mutex_unlock(&processador);
//      fim se
...
//        escolhe uma tarefa da fila de prontas
				processo_corrente = (queue_process_t *) queue_remove((queue_t **) &ready, (queue_t *) ready);
//        migra essa tarefa para o estado "rodando"
				processo_corrente->tempo_executado_quantum_atual = 0;
